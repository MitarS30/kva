...
export class ShippingCartComponent implements OnInit{
  ...
  ngOnInit(): void {
    ...
    this.shopCartService.ItemArrivalListUpdated.subscribe(ItemArrivalList => {
      this.itemArrivalList = ItemArrivalList.filter(orderTime => 
        orderTime.order.state === 'sending' && orderTime.order.user.email === this.user.email
      );
    });
    console.log("lista glupa: ");
    console.log(this.itemArrivalList);
  ...
  calculateProgress(order: Cart): void{
    // let found = this.itemArrivalList.find((item) => item.order.id == order.id)!;
